...
FlipBook.prototype = {
	...
	,removeBookmark: function(pageNum) {
		...
		while(_g1 < _g) {
			var i1 = _g1++;
			haxe.Log.trace(RunTime.book.bookmarks[i1].pageNum,{ fileName : "FlipBook.hx", lineNumber : 1573, className : "FlipBook", methodName : "removeBookmark"});
	...
	,onReadyTouchEnd: function(e) {
	}
	,onTouchEnd: function(e) {
		this.onReadyTouchEnd(e);
...
RunTime.loadBookInfo = function() {
	...
	if(deviceWidth > 480) {
		...
		RunTime.setMenuVisible(RunTime.flipBook.btnSns,false);
		RunTime.setMenuVisible(RunTime.flipBook.btnEmail,false);
		RunTime.setMenuVisible(RunTime.flipBook.btnSns,false);
	}
	var menuCount = 0;
	...
	if(RunTime.book.menuEmailVisible) menuCount += 1;
	if(RunTime.book.menuSnsVisible) menuCount += 1;
	if(RunTime.book.menuDownloadVisible) menuCount += 1;
	var hideIcon = false;
	if(RunTime.clientWidth < 480) hideIcon = true;
	var maxMenu = 5;
	if(js.Lib.window.navigator.userAgent.indexOf("iPhone") != -1) {
		hideIcon = true;
		if(RunTime.clientWidth < 480) maxMenu = 5; else maxMenu = 8;
	}
	if(hideIcon) {
		if(menuCount < maxMenu) RunTime.setMenuVisible(RunTime.flipBook.btnMask,RunTime.book.menuHighlightVisible);
		...
		if(RunTime.book.menuBookmarkVisible) menuCount += 1;
		if(menuCount < maxMenu) {
			RunTime.setMenuVisible(RunTime.flipBook.btnDownload,RunTime.book.menuDownloadVisible);
			menuCount += 1;
		}
...
core.HtmlHelper.toBookmarkNodeHtml = function(bookmark) {
	...
	if(!bookmark.onlyread) s += "<button onclick=\"removeBookmark(" + Std.string(bookmark.pageNum - 1) + ")\" style=\"float:right;margin:0px -2px;\">" + L.s("RemoveBookmark","Remove") + "</button>";
	s += "</li>";
	return s;
}
