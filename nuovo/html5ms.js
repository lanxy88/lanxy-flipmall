...
FlipBook.prototype = {
	...
	,onTouchMove: function(e) {
		...
		if(!this.checkCanZoom()) obj.preventDefault(); else {
		}
		if(this.zoomStatus == core.ZoomStatus.zoomed && this.startMoveGesture) {
			...
			obj.preventDefault();
			this.startMoveGesture = false;
		} else if(Math.abs(js.Lib.window.innerWidth - RunTime.clientWidth) > 10) {
			js.Lib.window.scrollBy(-(offsetX | 0),-(offsetY | 0));
			this.hideBottomBar();
			this.hideTopBar();
		} else {
		}
	...
	,onMouseTouchMove: function(e) {
		...
		if(this.startMoveGesture && Math.abs(js.Lib.window.innerWidth - RunTime.clientWidth) < 10) {
			return;
			this.zoomLeftPage.src = "";
			this.zoomLeftPage.style.display = "none";
			this.zoomRightPage.src = "";
	...
	,pointUp: function(e) {
		var obj = e;
		this.downPointCount--;
		if(this.downPointCount < 0) this.downPointCount = 0;
	}
	...
	,attachActions: function() {
		...
		this.mask.onmousemove = $bind(this,this.onMouseTouchMove);
		...
		this.mask.onmspointerdown = $bind(this,this.pointDown);
		this.mask.onmspointerup = $bind(this,this.pointUp);
		this.maskGesture = eval("new MSGesture();");
		this.maskGesture.target = eval("document.getElementById('mask')");
		this.mask.onmsgesturechange = $bind(this,this.onTouchMove);
		this.mask.onmsgestureend = $bind(this,this.pointUp);
		this.mask.onmsgesturestart = $bind(this,this.startGesture);
	...
	,getZoomOutCSS: function() {
		return this.zoomCSS + "-ms-transform:scale(" + 1 + " );-ms-transition: all 500ms ;" + "transform:scale(" + 1 + ");transition: all 500ms ;" + "top:" + 0 + "px;left:" + 0 + "px;";
	}
	,getZoomInMoveCSS: function(x,y,atonce) {
		if(atonce) return this.zoomCSS + "-ms-transform:scale(" + this.zoomLevel + " );-ms-transition: all 0ms ;" + "transform:scale(" + this.zoomLevel + " ) ;transition: all 0ms ;" + "top:" + (this.currentOrgY + y | 0) + "px;left:" + (this.currentOrgX + x | 0) + "px;"; else return this.zoomCSS + "-ms-transform:scale(" + this.zoomLevel + " );-ms-transition: all 500ms ;" + "transform:scale(" + this.zoomLevel + " ) ; transition: all 500ms ;" + "top:" + (this.currentOrgY + y | 0) + "px;left:" + (this.currentOrgX + x | 0) + "px;";
	}
	,getZoomInCSS: function() {
		return this.zoomCSS + "-ms-transform:scale(" + this.zoomLevel + " );-ms-transition: all 500ms ;" + "transform:scale(" + this.zoomLevel + ");transition: all 500ms ;";
	}
...
RunTime.init = function() {
	...
	js.Lib.window.document.body.onresize = RunTime.msOnResize;
...
core.HtmlHelper.toPopupImageHtml = function(item,success) {
	...
	if(item.popupWidth != null && item.popupHeight != null) {
		...
		s += "</div>";
		haxe.Log.trace(s,{ fileName : "HtmlHelper.hx", lineNumber : 360, className : "core.HtmlHelper", methodName : "toPopupImageHtml"});
	} else {
		s = "";
...
core.HtmlHelper.toPopupVideoHtml = function(item) {
	...
	var top = (RunTime.clientHeight - h) / 2 | 0;
	var s = "";
	s += "<div id=\"popupVideo\"style=\"position:absolute; z-index:201;left:" + Std.string(left) + "px; top:" + Std.string(top) + "px; width:" + Std.string(w) + "px; height:" + Std.string(h) + "px; background-color:#ffffff;transform: scale(0.2);transition: 1s ease-out;-ms-transform: scale(0.2);-ms-transition: 1s ease-out; -webkit-transform: scale(0.2); -webkit-transition: 1s ease-out; \">";
...
RunTime.urlIndex = "html5forms.html";
