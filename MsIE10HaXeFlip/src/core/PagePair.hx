...
class PagePair 
{
	...
	public function new(i:Int) 
	{
		...
		if (i == 0)	// 首页
		{
			...
			RunTime.flipBook.rightLockIcon.style.left = Std.int((rightPage.drawParams.dw - 128) / 2) + "px";
			RunTime.flipBook.rightLockIcon.style.top = Std.int((rightPage.drawParams.dh - 128) / 2) + "px";
		}
		else if (i == RunTime.book.pages.length - 1 && i%2 == 1) // 只有末页
		{
			...
			RunTime.flipBook.leftLockIcon.style.left = Std.int((leftPage.drawParams.dw - 128) / 2) + "px";
			RunTime.flipBook.leftLockIcon.style.top = Std.int((leftPage.drawParams.dh - 128) / 2) + "px";
		}
		else
		{
