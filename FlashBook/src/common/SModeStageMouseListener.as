package common
{
	...
	public class SModeStageMouseListener
	{
		private var mainPage:smain;
		...
		public function SModeStageMouseListener(mainPage:smain)
		{
			this.stage = mainPage;
		...
		private function onMouseDoubleClick(event:MouseEvent):void
		{
			switch(RunTime.MouseState)
			{
				case RunTime.MOUSE_STATE_NORMAL:
					if(isStagePointInBookArea(mouseDownPoint) == true 
						&& isStagePointInBookArea(mouseDownPoint) == true
		...
		private function onMouseDown(event:MouseEvent):void
		{
			...
			if(RunTime.note.length > 0)
			{
				...
				var book:SModeBook = mainPage.book;
				var pageId:int = book.currentPage;
				if(pageId == -1) return;
				var root:Point = book.localToGlobal(new Point(0,0));
				if(event.stageY > root.y
					&& event.stageY < book.localToGlobal(new Point(0,book.height-1)).y - 20
				)
				{
					if(pageId > -1)
					{
						var note:NoteRecord = new NoteRecord(
							RunTime.note, pageId, 
							(event.stageX - root.x) / (book.width),
							(event.stageY - root.y) / book.height
							,true);
						note.save();
		...
		{
			var root:Point = mainPage.book.localToGlobal(new Point());
			if(p.x < root.x || p.y < root.y || p.x > root.x + mainPage.book.width || p.y > root.y + mainPage.book.height)
			{
				return false;
			}
			else
		...
		private function onMouseUp(event:MouseEvent):void
		{
			...
			else if(RunTime.MouseState == RunTime.MOUSE_STATE_NORMAL)
			{
				...
				if(isInBookBg(event.target as DisplayObject) == true)
				{
					var p:Point = new Point(event.stageX,event.stageY);
					var p0:Point = new Point(0,0);
					var p1:Point = new Point(mainPage.book.width, 0);
			...
			switch(RunTime.MouseState)
			{
				...
				case RunTime.MOUSE_STATE_NOTE_MOVING:{
					...
					)
					{
						...
						var mid:Point = mainPage.book.localToGlobal(new Point(mainPage.book.width,0));
						...
						r.xPos = (d.x - rootX) / (mainPage.book.width );
		...
		private function isInBookBg(item:DisplayObject):Boolean
		{
			return mainPage.bookBg.contains(item);
		}
		...
		{
			var book:SModeBook = this.mainPage.book;
		...
		private function releaseMask(p:Point = null):void
		{
			...
			if(mask.visible == true)
			{
				...
				if(w > 10 && h > 10)
				{
					var wScale:Number = mainPage.bookCanvas.width / w;
					var hScale:Number = mainPage.bookCanvas.height / h;
