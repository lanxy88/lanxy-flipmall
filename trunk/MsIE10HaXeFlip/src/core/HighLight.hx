...
class HighLight 
{
	...
	public function clone():HighLight {
		DataTransform();
		var hl:HighLight = new HighLight();
		hl.x = this.x;
		hl.y = this.y;
		hl.width = this.width;
	...
	public function save():Void {
		...
		//if(guid == ""){
			guid = RunTime.kvPrex + "@$ht$@" + Date.now().getTime();
		//}
		//Lib.alert(guid);
		DataTransform();
	...
	public function loadToContext2D(context:CanvasRenderingContext2D):Void {
		...
		//context.fillRect(Std.int(this.x), Std.int(this.y), Std.int(this.width), Std.int(this.height));
	...
	public function hitTest(mouseX:Float, mouseY:Float):Bool {
		...
		/*
		if (mouseX < this.x || mouseY < this.y || mouseX > this.getRight() || mouseY > this.getBottom() )
			return false;
		return true;
		*/
	...
	public function click(popupXOffset:Float = 0,popupYOffset:Float = 0):Void
	{
		...
		Lib.document.getElementById("cvsOthers").innerHTML = HtmlHelper.toNotePopupHtml(this, "saveHighlightNote", "deleteHighlightNote");
