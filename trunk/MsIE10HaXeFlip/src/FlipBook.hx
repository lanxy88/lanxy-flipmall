...
class FlipBook
{
	...
	public var leftLockIcon:HtmlDom;
	public var rightLockIcon:HtmlDom;
	...
	public var maskGesture:Dynamic;
	
	public var downPointCount:Int = 0;
	
	
	public var zoomCSS:String = "";
	public var zoomLevel:Float =  1;
	public var moveDX:Int = 0;
	public var moveDY:Int = 0;
	public var currentOrgX:Int = 0;
	public var currentOrgY:Int = 0;
	...
	public function new() 
	{
		...
		//Lib.alert(RunTime.book.bgColor);
		//zoomCSS = "margin:0; padding:0; overflow:auto; height:100%;backgroundColor:" + RunTime.book.bgColor+";";
		
		zoomCSS = "position:absolute;left:0px;top:0px;width:100%;height:100%;overflow: hide;" 
	...
	public function ZoomWithCSS():Void
	{
		...
		if (zoomLevel > 3) {
			...
			zoom.style.cssText = getZoomOutCSS();
		...
		resetCSS();
		zoomAt(0, 0);
		zoom.style.cssText = getZoomInCSS();
	...
	public function getZoomInCSS():String {
		//trace("zoom in sacle:" + zoomLevel);
		return zoomCSS +  "-ms-transform:scale(" + zoomLevel + " );-ms-transition: all 500ms ;" 
		+ "transform:scale(" + zoomLevel + ");transition: all 500ms ;";
	}
	
	public function getZoomInMoveCSS(x:Int, y:Int, atonce:Bool):String {
		
		if (atonce) {
			//trace("getZoomInMoveCSS");
			return zoomCSS +  "-ms-transform:scale(" + zoomLevel + " );-ms-transition: all 0ms ;" 
			+ "transform:scale(" + zoomLevel + " ) ;transition: all 0ms ;"
			+ "top:" + Std.int(currentOrgY + y) + "px;left:" + Std.int(currentOrgX + x) + "px;";
		}
		else {
			return zoomCSS +  "-ms-transform:scale(" + zoomLevel + " );-ms-transition: all 500ms ;" 
			+ "transform:scale(" + zoomLevel + " ) ; transition: all 500ms ;"
	...
	public function getZoomOutCSS():String {
		
		return zoomCSS +  "-ms-transform:scale(" + 1 + " );-ms-transition: all 500ms ;" 
	...
	public function attachActions():Void
	{
		...
		//mask.onclick = this.onMouseDown;// this.onMsMouseDown;
		
		mask.onmousedown = this.onTouchStart;
		mask.onmousemove = onMouseTouchMove;// this.onTouchMove;
		mask.onmouseup = this.onTouchEnd;
		...
		//mask.onmspointermove  =  this.pointMove; 
		
		mask.onmspointerdown = this.pointDown;
		mask.onmspointerup = this.pointUp;
		
		maskGesture = Lib.eval("new MSGesture();");
		maskGesture.target = Lib.eval("document.getElementById('mask')");
		mask.onmsgesturechange = onTouchMove;
		mask.onmsgestureend = pointUp;
		mask.onmsgesturestart = this.startGesture;
		...
		btnSearch.onclick = this.onSearchClick;
		btnAutoFlip.onclick = this.onAutoFlipClick;
		btnShowTxt.onclick = this.onShowTxtClick;
		tbPage.onfocus = this.onTbPageFocus;
		btnZoom.ontouchstart = this.onZoomClick;
		
		btnMask.onclick = this.onButtonMaskClick;
		btnBookMark.onclick = this.onButtonBookmark;
		btnNote.onclick = this.onButtonNoteClick;
	...
	public function startGesture(e:Event):Void {
		//Lib.alert("startGesture");
		touchActive = true;
	...
	public function pointMove(e:Event):Void {
		Lib.alert("pointMove");
	}
	public function pointDown(e:Event):Void {
		var obj:Dynamic = e ;
		//Lib.alert(obj.pointerType);
		
		trace("pointDown");
		
		//if (obj.pointerType == 4) return;
		
		maskGesture.addPointer(obj.pointerId);
		
		downPointCount++;
		
		//trace("downPointCount=" + downPointCount);
		if (downPointCount > 1) {
			zoomAt(0, 0);
	...
	public function pointUp(e:Event):Void {
		var obj:Dynamic = e ;
		//Lib.alert(obj.pointerType);
		//maskGesture.addPointer(obj.pointerId);
		//Lib.alert("up");
		//trace("pointUP");
		downPointCount--;
		if (downPointCount < 0) downPointCount = 0;
	...
	public function onMsMouseDown(e):Void {
	...
	public function onMouseDown(e:Event):Void
	{
		...
		//Lib.alert(Lib.window.navigator.msPointerEnabled);
	...
	public function onMouseTouchStart(e:Event):Void
	{
	
		var obj:Dynamic = e;
		//obj.preventDefault();
		var touch:Dynamic = obj; // obj.touches[0];
		//选中高亮
		...
		*/
		...
		var centerX:Int = Std.int(Lib.document.body.clientWidth / 2);
		var centerY:Int = Std.int(Lib.document.body.clientHeight / 2);
		
		var _dx:Int = 0;// -Std.int(touch.pageX - centerX + currentOrgX / 2) ;
		var _dy:Int = 0;// -Std.int(touch.pageY - centerY + currentOrgY / 2);
		
		var xInBook:Int = touch.pageX;
		var yInBook:Int = touch.pageY;
		
		if (zoomLevel > 1) {
			_dx = -Std.int(touch.pageX - centerX + currentOrgX/(zoomLevel-1)) ;
			_dy = -Std.int(touch.pageY - centerY + currentOrgY/(zoomLevel - 1));
			
			xInBook = touch.pageX + _dx * (zoomLevel - 1) / zoomLevel;
			yInBook = touch.pageY + _dy * (zoomLevel - 1) / zoomLevel;
		}
		
		//onButtonLinkClick(touch.pageX, touch.pageY);
		if (onButtonLinkClick(xInBook, yInBook)) {
	...
	public function onMouseTouchMove(e:Event):Void
	{
		//Lib.alert("onMouseTouchMove");
		...
		if ( startMoveGesture && Math.abs(Lib.window.innerWidth - RunTime.clientWidth) < 10)
		{
			return ;
	...
	public function onTouchMove(e:Event):Void
	{
		...
		if (!checkCanZoom()) {
			...
			//var m:Dynamic = Lib.eval(" new MSCSSMatrix(e.target.style.msTransform);");
			//Lib.alert(m);
			//obj.target.style.msTransform = m.translate(obj.offsetX, obj.offsetY) // Move the transform origin under the center of the gesture
			//							.rotate(obj.rotation * 180 / Math.PI) // Apply Rotation
			//							.scale(obj.scale) // Apply Scale
			//							.translate(obj.translationX, obj.translationY) // Apply Translation
			//							.translate(-obj.offsetX, -obj.offsetY); // Move the transform origin back
			//return;
		}
		//Lib.alert("touch move:" + touchActive);
		
		//css放大状态
		
		if (zoomStatus == ZoomStatus.zoomed &&  startMoveGesture ) {
			if (currentOrgX + offsetX > (Lib.document.body.clientWidth / 2) ) {
				//Lib.alert("too len");
				offsetX = Lib.document.body.clientWidth / 2 - currentOrgX;
			}
			...
			if (currentOrgY + offsetY > (Lib.document.body.clientHeight /2) ) {
				offsetY = Lib.document.body.clientHeight / 2 - currentOrgY;
			}
			...
			zoom.style.cssText = getZoomInMoveCSS(Std.int(offsetX), Std.int(offsetY),true);
			trace(zoom.style.cssText);
			
			moveDX = Std.int(offsetX);
			moveDY = Std.int(offsetY);
		}
		else
		{
			if (touchActive && Math.abs(Lib.window.innerWidth - RunTime.clientWidth) < 10)
			{
				//return;
				//Lib.alert(Math.abs(Lib.window.innerWidth - RunTime.clientWidth));
				this.zoomLeftPage.src = "";
			...
			else if(Math.abs(Lib.window.innerWidth - RunTime.clientWidth) > 10) {
				//Lib.alert("zoom");
				...
				//							.translate(obj.translationX, obj.translationY) // Apply Translation
				//							.translate(-obj.offsetX, -obj.offsetY); // Move the transform origin back
				//Lib.alert(Lib.document.offsetTop);
				Lib.window.scrollBy( -Std.int(offsetX), -Std.int(offsetY));
				hideBottomBar();
				hideTopBar();
	...
	public function onEmailClick(e:Event):Void {
		...
		//Lib.window.document.getElementById("topBarContent").innerHTML = "abc";
