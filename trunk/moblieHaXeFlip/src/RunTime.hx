...
class RunTime 
{
	...
	public static var urlIndex:String = "html5forwebkit.html";
	...
	private static function loadBookInfo():Void
	{
		...
		if (book.menuEmailVisible) menuCount += 1;
		if (book.menuSnsVisible) menuCount += 1;
		if (book.menuDownloadVisible) menuCount += 1;
		...
		var maxMenu:Int = 5;
		if (Lib.window.navigator.userAgent.indexOf("iPhone") != -1)  {
			hideIcon = true;
		...
		if (hideIcon) {
			...
			if (book.menuBookmarkVisible) menuCount += 1;
			if (menuCount < maxMenu) {
				setMenuVisible(RunTime.flipBook.btnDownload, book.menuDownloadVisible);
				menuCount += 1;
			}
	...
	private static function loadPageInfo():Void
	{
		...
		while (i.hasNext() == true)
		{
			...
			page.urlPage = urlRoot + medium;
			page.urlBigPage = urlRoot + source;
			page.urlThumb = urlRoot + thumb;
			page.urlFullPage = urlRoot + source;
	...
	private static function loadSlideshow(ctx:Xml2Html):Void
	{
		...
		for (i in 0 ... slides.length)
		{
			...
			for (j in 0 ... pics.length)
			{
				...
				slide.url = urlRoot + pnode.getAttribute("url");
	...
	private static function loadHotlinks(ctx:Xml2Html):Void
	{
		...
		for (i in 0 ... links.length)
		{
			...
			try {
				...
				if (iframe!=null) {
					htmlText = "<iframe src=\""+iframe.getAttribute("src")+"\" frameborder=\"0\" style=\"width:100%;height:100%\" scrolling=\"yes\" ></iframe>";
				}
			}catch (ex:Dynamic) {
	...
	public static function loadButtons():Void
	{
		...
		while (i.hasNext() == true)
		{
			...
			try {
				...
				if (iframe!=null) {
					htmlText = "<iframe src=\""+iframe.get("src")+"\" frameborder=\"0\" style=\"width:100%;height:100%\" scrolling='yes'></iframe>";
				}
			}catch (ex:Dynamic) {
