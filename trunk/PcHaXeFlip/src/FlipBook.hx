...
class FlipBook
{
	 ...
	 * 忽略MouseDown
	 */
	public var bAbortMouseDown:Bool = false;
	...
	public function new() 
	{
		...
		//zoomCSS = "margin:0; padding:0; overflow:auto; height:100%;";
	}
	...
	public function getZoomInCSS():String {
		...
		return zoomCSS +  "-webkit-transform:scale(" + zoomLevel + " );-webkit-transition: all 500ms ;" 
		+ "-moz-transform:scale(" + zoomLevel + ");-moz-transition: all 500ms ;";
	}
	
	public function getZoomInMoveCSS(x:Int, y:Int, atonce:Bool):String {
		
		if(atonce){
			return zoomCSS +  "-webkit-transform:scale(" + zoomLevel + " );-webkit-transition: all 0ms ;" 
			+ "-moz-transform:scale(" + zoomLevel + " ) ; -moz-transition: all 0ms ;"
	...
	public function getZoomOutCSS():String {
		return zoomCSS +  "-webkit-transform:scale(" + 1 + " );-webkit-transition: all 500ms ;" 
		+ "-moz-transform:scale(" + 1 + ");-moz-transition: all 500ms ;"
		+ "top:" + 0 + "px;left:" + 0 +"px;";
	}
	...
	public function attachActions():Void
	{
		...
		mask.onmousemove = this.onTouchMove;
	...
	public function onMouseDown(e:Event):Void
	{
		...
		//firefox工具栏不可取消
		if (Lib.window.navigator.userAgent.indexOf("Firefox") != -1) {
			return;
		...
		if (bStartHighLight || bStartNote || bAbortMouseDown) return;
		if (zoomStatus == ZoomStatus.zoomed) {
			this.hideBottomBar(e);
			return;
		}
		//Lib.alert(bAbortMouseDown);
	...
	public function onDblClick(e:Event):Void {
		...
		//zoom.style.cssText += "-webkit-transform:scale(2);-webkit-transition: all 800ms ;-moz-transform:scale(2);-moz-transition: all 800ms ;";
	}
	...
	public function onTouchEnd(e:Event):Void
	{
		...
		//moveDX = e.clientX;// Std.int(gestureLastX);
		//moveDY = e.clientY;// Std.int(gestureLastY);
	...
	public function onMouseTouchMove(e:Event):Void
	{
		...
		if (zoomStatus != ZoomStatus.zoomed &&  startMoveGesture && Math.abs(Lib.window.innerWidth - RunTime.clientWidth) < 10)
		{
	...
	public function onTouchMove(e:Event):Void
	{
		...
		if (zoomStatus == ZoomStatus.zoomed &&  obj.touches.length==1 ) {
			
			//trace("currentOrgX + offsetX=" + (currentOrgX + offsetX));
			//trace("Lib.document.clientWidth / 2=" + Lib.document.body.clientWidth / 2);
			...
			zoom.style.cssText = getZoomInMoveCSS(Std.int(offsetX), Std.int(offsetY),false);
			
			moveDX = Std.int(offsetX);
			moveDY = Std.int(offsetY);
		}
		
		else if (zoomStatus != ZoomStatus.zoomed && obj.touches.length==1 && Math.abs(Lib.window.innerWidth - RunTime.clientWidth) < 10)
		{
			//Lib.alert(Math.abs(Lib.window.innerWidth - RunTime.clientWidth));
			this.zoomLeftPage.src = "";
			this.zoomLeftPage.style.display = "none";
